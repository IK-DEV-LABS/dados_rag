...
def get_long_path(path):
    ...
    if platform.system() == 'Windows':
        # Aplica o prefixo \\?\ corretamente para lidar com caminhos longos no Windows
        return '\\\\?\\' + os.path.abspath(path).strip()
    else:
        # Retorna o caminho normal para sistemas baseados em Unix
...
class SharePointClient:
    def __init__(self, tenant_id, client_id, client_secret, resource_url):
        ...
        self.access_token = self.get_access_token()  # Inicializa e armazena o token de acesso na instanciação
    ...
    """
        # Corpo para a solicitação do token de acesso
        body = {
            'grant_type': 'client_credentials',
            'client_id': self.client_id,
            'client_secret': self.client_secret,
            'scope': f"{self.resource_url.rstrip('/')}/.default"
        }
        response = requests.post(self.base_url, headers=self.headers, data=body)
        data = response.json()
        if 'access_token' not in data:
            print(f"Error acquiring token: {data}")
        return data.get('access_token')  # Extrai o token de acesso da resposta
    ...
    """
        # Constrói URL para solicitar ID do site
        ...
        data = response.json()
        if 'id' not in data:
            print(f"Error getting site ID: {data}")
        return data.get('id')  # Retorna o ID do site
    ...
    """
        # Recupera IDs e nomes de drives associados a um site
    ...
    def get_folder_id(self, site_id, drive_id, folder_path):
        ...
        # Divide o caminho da pasta em pastas individuais
        folders = folder_path.split('/')

        # Começa com a pasta raiz
        current_folder_id = 'root'

        # Percorre cada pasta no caminho
        for folder_name in folders:
            # Constrói a URL para acessar o conteúdo da pasta atual
            ...
            # Percorre os itens e encontra a pasta
            for item in items_data['value']:
                if 'folder' in item and item['name'] == folder_name:
                    # Atualiza o ID da pasta atual e interrompe o loop
                    current_folder_id = item['id']
                    break
            else:
                # Se a pasta não foi encontrada, retorna None
                return None

        # Retorna o ID da pasta final no caminho
    ...
    """
        ...
        while folder_contents_url:
                ...
                for item in folder_contents.get('value', []):
                    ...
                    # Modifique a URL do site para apontar para o elemento específico
    ...
    """
        ...
        if response.status_code == 200:
            ...
            full_path = get_long_path(full_path)  # Aplica a correção de caminho longo condicionalmente com base no SO
    ...
    """
        # Baixa recursivamente todo o conteúdo de uma pasta
        folder_contents_url = f'https://graph.microsoft.com/v1.0/sites/{site_id}/drives/{drive_id}/items/{folder_id}/children'
            ...
            for item in folder_contents['value']:
                if 'folder' in item:
                    ...
                    self.download_folder_contents(site_id, drive_id, item['id'], new_path,
                                                  level + 1)  # Chamada recursiva para subpastas
                elif 'file' in item:
                    file_name = item['name']
    ...
    def download_file_contents(self, site_id, drive_id, file_id, local_save_path):
        ...
        try:
            ...
            # Obter URL de download e nome do arquivo
            download_url = file_data['@microsoft.graph.downloadUrl']
            file_name = file_data['name']
            sharepoint_file_path = file_data['parentReference']['path']  # Este é o caminho do arquivo SharePoint
            index = sharepoint_file_path.find(":/")

            # Extrair tudo após ":/"
            if index != -1:
                extracted_path = sharepoint_file_path[index+2:]  # Adicionando 2 para pular os caracteres ":/"
                local_save_path = local_save_path + "/" + extracted_path
                os.makedirs(local_save_path, exist_ok=True) # criar subpasta local
            else:
                extracted_path = ""
            ...
            # Se nenhuma exceção foi levantada, o download do arquivo foi bem-sucedido
            return True
    ...
    def download_all_files(self, site_id, drive_id, local_folder_path, sharepoint_path="root", extensions=None):
        """
        ...
        Args:
            ...
            extensions (list, optional): List of file extensions to download (e.g. ['.pdf']).
        """
        try:
            root_sharepoint_path = None
            if sharepoint_path != "root":
                folder_id = self.get_folder_id(site_id, drive_id, sharepoint_path)
                if not folder_id:
                    print(f"Folder not found: {sharepoint_path}")
                    return
                root_sharepoint_path = sharepoint_path
            else:
                folder_id = sharepoint_path

            self.recursive_download(site_id, drive_id, folder_id, local_folder_path, root_sharepoint_path, extensions)
        except Exception as e:
            print(f"An error occurred while downloading files: {e}")

    def recursive_download(self, site_id, drive_id, folder_id, local_path, root_sharepoint_path=None, extensions=None):
        """
        ...
        Args:
            ...
            root_sharepoint_path (str): The base path to subtract from item paths to maintain relative structure.
        ...
        try:
            ...
            for item in folder_contents:
                ...
                # Adjust path relative to the root folder being downloaded
                if root_sharepoint_path:
                    # Normalize paths to handle potential slash differences
                    norm_root = root_sharepoint_path.replace('\\', '/').strip('/')
                    norm_path = sharepoint_path.replace('\\', '/').strip('/')
                    
                    if norm_path.startswith(norm_root):
                        # Strip the root path from the item path
                        sharepoint_path = norm_path[len(norm_root):].lstrip('/')

                new_local_path = os.path.normpath(os.path.join(local_path, sharepoint_path))
                # Garante que o diretório local exista antes de baixar
                #"data2\\BASIC STUDIES\\1. Cross Category
                #os.makedirs("data2\\BASIC STUDIES\\1. Cross Category", exist_ok=True)
                
                os.makedirs(new_local_path, exist_ok=True)
                if item['type'] == 'folder':
                    self.recursive_download(site_id, drive_id, item['id'], local_path, root_sharepoint_path, extensions)
                elif item['type'] == 'file':
                    if extensions:
    ...
    """
        # Obter a URL de download e o nome do arquivo consultando a API do Microsoft Graph
        file_url = f'https://graph.microsoft.com/v1.0/sites/{site_id}/drives/{drive_id}/items/{file_id}'
        headers = {'Authorization': f'Bearer {self.access_token}'}  # Usar o token de acesso armazenado para autorização
        response = requests.get(file_url, headers=headers)  # Fazer a solicitação HTTP para obter detalhes do arquivo
        file_data = response.json()  # Analisar a resposta JSON para obter dados do arquivo
        # print(file_data)

        download_url = file_data['@microsoft.graph.downloadUrl']  # Extrair a URL de download direto da resposta

        # Obter o conteúdo do arquivo da URL de download
        response = requests.get(download_url, headers=headers)  # Fazer a solicitação HTTP para baixar o arquivo

        # Criar um objeto BytesIO a partir do conteúdo da resposta, o que permite ler e escrever bytes na memória
        stream = io.BytesIO(response.content)  # Isso é útil para lidar com dados binários como arquivos sem salvar em disco

        # Verificar o tipo de arquivo e usar o carregador personalizado apropriado para lidar com o conteúdo do arquivo
        if file_type == 'application/pdf':
            # Usar CustomPDFLoader para lidar com arquivos PDF; ele inicializa com o fluxo e o nome do arquivo
            loader = CustomPDFLoader(stream, file_name)
            return loader
        elif file_type == 'application/vnd.openxmlformats-officedocument.wordprocessingml.document':
            # Usar CustomWordLoader para documentos Word para lidar e potencialmente dividir o conteúdo do documento
            loader = CustomWordLoader(stream, file_name)
            return loader
        elif file_type == 'application/vnd.openxmlformats-officedocument.presentationml.presentation':
            # Usar CustomPPTLoader para apresentações PowerPoint para ler e dividir a apresentação em slides
            loader = CustomPPTLoader(stream, file_name)
            return loader
        elif file_type == 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet':
            # Usar CustomExcelLoader para planilhas Excel para ler e possivelmente dividir as planilhas em partes separadas
            loader = CustomExcelLoader(stream, file_name)
            return loader
        elif file_type in ['text/csv', 'text/plain']:
            # Usar CustomTextLoader para arquivos de texto simples ou CSV para lidar e dividir o texto conforme necessário
            loader = CustomTextLoader(stream, file_name)
            return loader
        else:
            print(f"Unsupported file type: {file_type}")
            pass  # Espaço reservado para tipos de arquivos adicionais que podem precisar ser implementados no futuro
...
class CustomPDFLoader(BaseLoader):
    ...
    def __init__(self, stream: BytesIO, filename: str, password: Optional[Union[str, bytes]] = None,
        ...
        # Inicializa com um fluxo binário, nome do arquivo, senha opcional e um sinalizador de extração de imagem
        self.stream = stream
        self.filename = filename
        # Inicializa um analisador de PDF com proteção por senha opcional e configurações de extração de imagem
        self.parser = PyPDFParser(password=password, extract_images=extract_images)

    def load(self) -> List[Document]:
        # Converte o fluxo binário em um objeto Blob que é exigido pelo analisador
        blob = Blob.from_data(self.stream.getvalue())
        # Analisa o PDF e converte cada página ou segmento em um objeto de documento separado
        documents = list(self.parser.parse(blob))

        # Adiciona o nome do arquivo como metadados a cada documento para identificação
...
class CustomWordLoader(BaseLoader):
    ...
    def __init__(self, stream, filename: str):
        # Inicializa com um fluxo binário e nome do arquivo
        self.stream = stream
        self.filename = filename

    def load_and_split(self, text_splitter=None):
        # Usa python-docx para analisar o documento Word a partir do fluxo binário
        doc = DocxDocument(self.stream)
        # Extrai e concatena todos os textos de parágrafo em uma única string
        text = "\n".join([p.text for p in doc.paragraphs])

        # Verifica se um utilitário divisor de texto é fornecido
        if text_splitter is not None:
            # Usa o divisor fornecido para dividir o texto em documentos gerenciáveis
            split_text = text_splitter.create_documents([text])
        else:
            # Sem um divisor, trata todo o texto como um único documento
            split_text = [{'text': text, 'metadata': {'source': self.filename}}]

        # Adiciona metadados de origem a cada documento resultante
...
class CustomExcelLoader(BaseLoader):
    ...
    def load_and_split(self, text_splitter=None):
        # Usa pandas para carregar o arquivo Excel a partir do fluxo binário
        xls = pd.ExcelFile(self.stream, engine='openpyxl')
        # Obtém a lista de todos os nomes de planilhas na pasta de trabalho
        sheet_names = xls.sheet_names

        split_sheets = []
        for sheet in sheet_names:
            # Analisa cada planilha em um DataFrame
            df = xls.parse(sheet)
            # Converte o DataFrame em uma única string com cada valor de célula separado por novas linhas
            text = '\n'.join(df.values.astype(str).flatten().tolist())

            # Verifica se um divisor de texto é fornecido para dividir ainda mais o conteúdo da planilha
            if text_splitter is not None:
                # Usa o divisor para criar documentos a partir do texto
                split_text = text_splitter.create_documents([text])
                ...
                # Sem um divisor, trata todo o texto da planilha como um único documento
...
class CustomTextLoader(BaseLoader):
    ...
    def load_and_split(self, text_splitter=None):
        # Usa chardet para detectar a codificação do fluxo
